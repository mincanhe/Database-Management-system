...
public class SchemaManager implements Serializable {
	private final static int MAX_NUM_CREATING_RELATIONS = 100;
	private MainMemory mem;
	private Disk disk;
	private TreeMap<String, Integer> relation_name_to_index;
	private Relation[] relations = new Relation[MAX_NUM_CREATING_RELATIONS];
	Schema[] schemas = new Schema[MAX_NUM_CREATING_RELATIONS];
	private int offset;
	...
	public SchemaManager(MainMemory mem, Disk disk) {
		...
		relation_name_to_index = new TreeMap<>();
	}
	...
	public Schema getSchema(String relation_name) {
		if (!relation_name_to_index.containsKey(relation_name)) {
			System.err.print("getSchema ERROR: relation " + relation_name + " does not exist\n");
			return new Schema();
		} else {
			return new Schema(schemas[relation_name_to_index.get(relation_name)]);
		}
	}
	...
	public Relation createRelation(String relation_name, Schema schema) {
		if (relation_name.equals("")) {
			System.err.print("createRelation ERROR: empty relation name\n");
			return null;
		}
		if (relation_name_to_index.containsKey(relation_name)) {
			System.err.print("createRelation ERROR: " + relation_name + " already exists\n");
			return null;
		}
		if (schema.isEmpty()) {
		...
		if (offset == MAX_NUM_CREATING_RELATIONS) {
			System.err.print("createRelation ERROR: no more " + "relations can be created.\n");
			return null;
		}
		relation_name_to_index.put(relation_name, offset);
		relations[offset] = new Relation(this, offset, relation_name, mem, disk);
	...
	public Relation getRelation(String relation_name) {
		if (!relation_name_to_index.containsKey(relation_name)) {
			System.err.print("getRelation ERROR: relation " + relation_name + " does not exist\n");
			return null;
		} else {
			return relations[relation_name_to_index.get(relation_name)];
		}
	}
	...
	public boolean deleteRelation(String relation_name) {
		if (!relation_name_to_index.containsKey(relation_name)) {
			System.err.print("deleteRelation ERROR: relation " + relation_name + " does not exist\n");
			return false;
		}
