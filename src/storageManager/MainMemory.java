...
public class MainMemory implements Serializable {
    ...
    public Block getBlock(int memory_block_index) {
        if (memory_block_index < 0 || memory_block_index >= Config.NUM_OF_BLOCKS_IN_MEMORY) {
            System.err.print("getBlock ERROR: block index " + memory_block_index + " out of memory bound\n");
            return null;
        }
    ...
    public boolean setBlock(int memory_block_index, Block b) {
        if (memory_block_index < 0 || memory_block_index >= Config.NUM_OF_BLOCKS_IN_MEMORY) {
            System.err.print("setBlock ERROR: block index " + memory_block_index + " out of memory bound\n");
            return false;
        }
    ...
    boolean setBlocks(int memory_block_index, ArrayList<Block> vb) {
        if (memory_block_index < 0 || memory_block_index >= Config.NUM_OF_BLOCKS_IN_MEMORY) {
            System.err.print("setBlocks ERROR: block index " + memory_block_index + " out of memory bound\n");
            return false;
        }
        if ((memory_block_index + vb.size() - 1) >= Config.NUM_OF_BLOCKS_IN_MEMORY) {
            System.err.print("setBlocks ERROR: number of blocks " + vb.size() + " out of memory bound\n");
            return false;
        }
    ...
    public ArrayList<Tuple> getTuples(int memory_block_begin, int num_blocks) {
        if (memory_block_begin < 0 || memory_block_begin >= Config.NUM_OF_BLOCKS_IN_MEMORY) {
            System.err.print("getTuples ERROR: block index " + memory_block_begin + " out of memory bound\n");
            return new ArrayList<>();
        }
        if (num_blocks <= 0) {
            System.err.print("getTuples ERROR: num of blocks " + num_blocks + " too few\n");
            return new ArrayList<>();
        }
        int i;
        if ((i = memory_block_begin + num_blocks - 1) >= Config.NUM_OF_BLOCKS_IN_MEMORY) {
            System.err.print("getTuples ERROR: access to block " + "out of memory bound: " + i + "\n");
            return new ArrayList<>();
        }
        ArrayList<Tuple> tuples = new ArrayList<>();
        Schema s = blocks[memory_block_begin].getTuples().get(0).getSchema();
        for (i = memory_block_begin; i < memory_block_begin + num_blocks; i++) {
            ...
            if (!tuples2.get(0).getSchema().equals(s)) {
                System.err.print("getTuples ERROR: schema at memory " + "block " + i + " has a different schema\n");
                return new ArrayList<>();
            }
    ...
    public boolean setTuples(int memory_block_begin, ArrayList<Tuple> tuples) {
        if (memory_block_begin < 0 || memory_block_begin >= Config.NUM_OF_BLOCKS_IN_MEMORY) {
            System.err.print("setTuples ERROR: block index " + memory_block_begin + " out of memory bound\n");
            return false;
        }
        ...
        if (memory_block_begin + num_blocks + num_additional_blocks > Config.NUM_OF_BLOCKS_IN_MEMORY) {
            System.err.print("setTuples ERROR: number of tuples " + "exceed the memory space\n");
            return false;
        }
        int t1 = 0, t2 = 0;
        int i;
