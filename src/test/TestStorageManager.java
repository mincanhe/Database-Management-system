...
public class TestStorageManager {
	...
	private static void appendTupleToRelation(Relation relation_reference, MainMemory mem, int memory_block_index,
		...
		if (relation_reference.getNumOfBlocks() == 0) {
			...
			if (block_reference.isFull()) {
				...
				relation_reference.setBlock(relation_reference.getNumOfBlocks() - 1, memory_block_index); // write
				// back
				// to
				// the
				// relation
	...
	public static void main(String[] args) {
		...
		SchemaManager schemaManager = new SchemaManager(mem, disk);
		...
		// Schema schema_error4 = new Schema(field_names, field_types); //
		// ArrayList
		// sizes
		// unmatched
		...
		Relation relation_reference = schemaManager.createRelation(relation_name, schema);
		...
		// Error testing
		System.out.print("Error testing: \n");
		schemaManager.createRelation(relation_name, schema); // create a
		// relation with
		// the same name
		schemaManager.createRelation("test", new Schema()); // create a
		// relation with
		// empty schema
		System.out.print("\n");
		...
		System.out.print(schemaManager + "\n");
		System.out.print("From the schema manager, the table " + relation_name + " exists: "
				+ (schemaManager.relationExists(relation_name) ? "TRUE" : "FALSE") + "\n");
		System.out.print("From the schema manager, the table " + relation_name + " has schema:\n");
		System.out.print(schemaManager.getSchema(relation_name) + "\n");
		System.out.print("From the schema manager, the table " + relation_name + " has schema:\n");
		System.out.print(schemaManager.getRelation(relation_name).getSchema() + "\n");

		System.out.print("Creating table ExampleTable2 with the same schema\n");
		schemaManager.createRelation("ExampleTable2", schema);
		...
		System.out.print(field_types.toString() + "\n");
		relation_reference = schemaManager.createRelation("ExampleTable3", schema3);
		System.out.print("After creating a realtion, current schemas and relations: \n");
		System.out.print(schemaManager + "\n");

		System.out.print("Deleting table ExampleTable2\n");
		schemaManager.deleteRelation("ExampleTable2");
		...
		// Error testing
		System.out.print("Error testing: \n");
		System.out.print("The table ExampleTable2 exists: "
				+ (schemaManager.relationExists("ExampleTable2") ? "TRUE" : "FALSE") + "\n");
		schemaManager.createRelation("", schema); // empty relation name
		schemaManager.getSchema("ExampleTable2"); // relation does not exist
		schemaManager.getRelation("ExampleTable2"); // relation does not exist
		schemaManager.deleteRelation("ExampleTable2"); // relation does not
		// exist
		System.out.print("\n");
		...
		Tuple tuple = relation_reference.createTuple(); // The only way to
		// create a tuple is to
		// call "Relation"
		...
		// Error testing
		System.out.print("Error testing: \n");
		Tuple tuple2 = schemaManager.getRelation("ExampleTable1").createTuple();
		...
		block_reference.setTuple(tuple.getTuplesPerBlock(), tuple); // out of
		// bound
		...
		block_reference.invalidateTuple(tuple.getTuplesPerBlock()); // out of
		// bound
		...
		// append the tuple to the end of the ExampleTable3 using memory block 5
		// as output buffer
		...
		relation_reference.getBlock(relation_reference.getNumOfBlocks(), 5); // out
		// of
		// bound
		...
		relation_reference.getBlocks(relation_reference.getNumOfBlocks(), 5, 1); // out
		// of
		// bound
		...
		relation_reference.deleteBlocks(relation_reference.getNumOfBlocks()); // out
		// of
		// bound
		schemaManager.getRelation("ExampleTable1").setBlock(0, 0); // different
		// table
		// schemas
		schemaManager.getRelation("ExampleTable1").setBlocks(0, 0, 1); // different
		// table
		// schemas
		System.out.print("\n");
		...
		mem.getTuples(1, 2); // Error: get memory block 1-2, which contain
		// tuples of different tables
